...
function ChatSidebar() {
  ...
  const conversationUsers = (Array.isArray(conversations) ? conversations : [])
    ...
    >
      ...
      >
        <div className="shrink-0 border-b border-border px-3 pb-2 pt-2">
          <SearchField
            fullWidth
            aria-label="Search users and chats"
            variant="secondary"
            className="w-full"
            value={searchQuery}
        ...
        >
          {filteredConversations.length === 0 ? (
            <div className="px-4 py-8 text-center">
              <p className="text-sm text-muted mb-2">No active conversations yet.</p>
              <button
                type="button"
                onClick={() => setSidebarTab("users")}
                className="inline-flex items-center gap-1.5 rounded-lg bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary transition hover:bg-primary/20 cursor-pointer"
              >
                <UsersIcon className="size-3.5" />
                View all users to chat
              </button>
            </div>
          ) : (
